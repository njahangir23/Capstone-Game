...
@onready var speech_bubble := $SpeechBubble   # 👈 updated name


func say(text: String) -> void:
	if speech_bubble:
		speech_bubble.set_text(text, 3.0)
	else:
		print("speech_bubble is NULL!")
...
func _on_ditch_trigger_body_entered(body: Node2D) -> void:
	print("DitchTrigger entered by: ", body.name)

	if body == self:
		print("Farmer detected in ditch!")
		say("Oh shit!!")
